        ...
        while AnimationThreadStatus and (IsInAnimation or PtfxPrompt) do
            ...
            if PtfxPrompt and CurrentAnimOptions then
                ...
                if not PtfxNotif then
                    SimpleNotify(CurrentAnimOptions.PtfxInfo or Translate('ptfxinfo'), "info")
                    PtfxNotif = true
                end
...
function OnEmotePlayGeneration(name)
    local playerPed = GetPlayerPed(-1)
    cleanScenarioObjects(false)
    ClearPedTasks(playerPed)
    DestroyAllProps(false)
    Wait(150)
    ...
    if emoteData.scenario then
        ...
        if emoteData.scenarioType == ScenarioType.MALE then
            if PlayerGender == "male" then
                TaskStartScenarioInPlace(playerPed, emoteData.scenario, 0, true)
            end
        elseif emoteData.scenarioType == ScenarioType.OBJECT then
            local BehindPlayer = GetOffsetFromEntityInWorldCoords(playerPed, 0.0, -0.5, -0.5)
            TaskStartScenarioAtPosition(playerPed, emoteData.scenario, BehindPlayer.x, BehindPlayer.y, BehindPlayer.z, GetEntityHeading(playerPed), 0, true, false)
    ...
    TaskPlayAnim(playerPed, emoteData.dict, emoteData.anim, 5.0, 5.0, animOption and animOption.EmoteDuration or -1, flag, 0, false, false, false)
    RemoveAnimDict(emoteData.dict)

    if not animOption or not animOption.Prop then return end
    addProps(animOption, nil, false)
end
...
function OnEmotePlay(name, textureVariation)
    ...
    -- don't replay the same emote
    if LocalPlayer.state.currentEmote == name then
        return
    end
    ...
    ClearPedTasks(PlayerPedId())
    Wait(200)
    TaskPlayAnim(PlayerPedId(), emoteData.dict, emoteData.anim, animOption?.BlendInSpeed or 5.0, animOption?.BlendOutSpeed or 5.0, animOption?.EmoteDuration or -1, animOption?.Flag or movementType or 49, 0, false, false, -- make player be able to walk by default (movementType 49)
        false)
    RemoveAnimDict(emoteData.dict)
